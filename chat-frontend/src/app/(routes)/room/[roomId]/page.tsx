...
export default function MainPage({ initialChats, upVotes1 = 3, upVotes2 = 10 }: MainPageProps) {
  ...
  const closeModal = () => setIsModalOpen(false);
    ...
    function dismissChat(chatId: string) {
      setChats(chats => chats.filter(c => c.chatId !== chatId));
    }
                  ...
                  .map((chat, i) => (
                    <div className="flex flex-col gap-1 p-2" key={i}>
                      ...
                      <div className="flex gap-1 justify-between">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => dismissChat(chat.chatId)} className="text-xs text-red-500">Dismiss</button>
                          <button
                            className={`text-xs text-gray-400 ${isUpvoteCoolDown ? 'opacity-50 cursor-not-allowed' : ''}`}
                            onClick={() => {
                              const newChats = [...chats];
